...
function fetchEntries () {
  fetch('api.php?action=list', {
    ...
    .then(res => {
      if (!res.ok) throw new Error('No more entries')
      return res.json()
    })
    .then(data => {
      ...
      document.getElementById('next').disabled = offset + 4 >= totalEntries
      document.getElementById('prev').disabled = offset === 0
    })
    .catch(err => {
      alert('Nu mai ai ce sa vezi boss astea s toate gen')
      offset = Math.max(0, offset - 4) // rollback to previous valid offset
    })
}
